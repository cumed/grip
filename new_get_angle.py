...
pf.makePointFile("curvetest5Scopic.svg")
...
def bend_angle_cos(flag,prev_x, prev_y, curr_x, curr_y, next_x, next_y):
    ...
    quadrant = find_quadrant(curr_x - prev_x,curr_y - prev_y)
    if quadrant == 1 or 4 :
...
#            angle =-(np.pi + np.arccos(round((a**2 + c**2 - b**2) /(2*a*c),5)))
    ...
    if(next_x - curr_x == 0): 
        ...
        #print('AUGHHHHRHTHHGH')
        alpha = np.pi/2
                        #alpha = 0
    else:
        alpha = abs(np.arctan(float((next_y - curr_y)/(next_x - curr_x))))
    x0 = prev_x - next_x
    y0 = prev_y - next_y
   
    [x0, y0] = rotate(x0, y0, alpha, quadrant)
    if(x0 == 0):
        if (y0 >= 0):
              ang = 0
                            #print('NIGHTMARESSSSS')
                            #ang = np.pi/2
                          
        else:
            ang = 3*np.pi/2
    else:
        ang = np.arctan(float(y0/x0))
...
#    print(dat1[2])
    for i in range(0, len(dat1)-1):
        ...
        #theta[i] = bend_angle(rotate_flag[i],dat1[i-1][0],dat1[i][0],dat1[i-1][1],dat1[i][1],dat1[i-1][2],dat1[i][2])
            ...
            if i == 13:
                theta[i] = bend_angle_cos(1,prev_x, prev_y, curr_x, curr_y, next_x, next_y)
    ...
    plt.plot(bad_points[:,0], bad_points[:,1], 'rx')
